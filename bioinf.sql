...
/*!50001 VIEW `viewer` AS select concat(`modorg`.`genus`,' ',`modorg`.`species`) AS `organism`,`class`.`name` AS `name`,`gene`.`acc` AS `acc`,`gene`.`len` AS `len`,`gene`.`descr` AS `descr` from ((`gene` join `modorg` on((`modorg`.`id` = `gene`.`mo_id`))) join `class` on((`modorg`.`class_id` = `class`.`id`))) */;
...
-- Dump completed on 2016-12-08 16:39:13
